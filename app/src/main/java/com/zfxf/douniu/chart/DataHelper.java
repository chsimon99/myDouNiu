...
//            用（DIF-DEA）*2即为MACD柱状图。
    ...
    static void calculateVOL(List<KLineEntity> datas) {
        ...
        for (int i = 0; i < datas.size(); i++) {
            KLineEntity point = datas.get(i);
            final float pointVolume = point.getVolume();

            ma5 += pointVolume;
            ma10 += pointVolume;
            ma20 += pointVolume;
            if (i >= 5) {
                ma5 -= datas.get(i - 5).getVolume();
                point.MA5Vol = ma5 / 5f;
            } else {
                point.MA5Vol = ma5 / (i + 1f);
            }
            if (i >= 10) {
                ma10 -= datas.get(i - 10).getVolume();
                point.MA10Vol = ma10 / 10f;
            } else {
                point.MA10Vol = ma10 / (i + 1f);
            }
            if (i >= 20) {
                ma20 -= datas.get(i - 20).getVolume();
                point.MA20Vol = ma20 / 20f;
            } else {
                point.MA20Vol = ma20 / (i + 1f);
            }
        }
    }

    /**
     * 计算MA BOLL RSI KDJ MACD VOL
     *
     * @param datas
     */
